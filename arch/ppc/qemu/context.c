 ...
 *
 * PReP residual data: see residual.c
 */
...
arch_init_program(void)
{
    ...
    if (!is_apple()) {
        ...
        /* The malloc arena is mapped 1:1, so this is a physical address */
        res = residual_build(loadbase, loadsize);
        ctx->regs[REG_R3] = (uintptr_t)res;
